...
class ScreenRecorder {
  ...
  async getVideoPath () {
    return (await fs.exists(this._videoPath)) ? this._videoPath : null;
  }
...
commands.stopRecordingScreen = async function stopRecordingScreen (options = {}) {
  ...
  const videoPath = this._screenRecorder.isRunning()
    ? await this._screenRecorder.stop()
    : await this._screenRecorder.getVideoPath();
  if (!videoPath) {
    return '';
  }
